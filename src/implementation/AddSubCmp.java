 ...
 * @author millerti
 */
public class AddSubCmp extends FunctionalUnitBase {
    public AddSubCmp(IModule parent, String name) {
        super(parent, name);
    }
...
//            super(parent, "in:Math");  // this would be fine too
        ...
        public void compute(Latch input, Latch output) {
            ...
            if(ins.getOpcode().toString().equals("FADD"))
            {
            float result = source1 + source2;
            output.setResultFloatValue(result);
            } 
            
            if(ins.getOpcode().toString().equals("FSUB"))
            {
             float result = source1 - source2;
            output.setResultFloatValue(result);
            }
           
             if(ins.getOpcode().toString().equals("FCMP"))
            {
             float result = source1 - source2;
            output.setResultFloatValue(result);
            }
