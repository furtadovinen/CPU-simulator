 ...
 * @author millerti
 */
public class Memory extends FunctionalUnitBase {
    public Memory(IModule parent, String name) {
        super(parent, name);
    }
          ...
          public void compute(Latch input, Latch output) {
            ...
            switch (ins.getOpcode()) {
                case LOAD:
                    // Fetch the value from main memory at the address
                    // retrieved above.
                    value = memory[GlobalData.addrVal];
                    output.setResultValue(value);
                    output.setInstruction(ins);
                    addStatusWord("Mem[" + GlobalData.addrVal + "]");
                    break;
                
                case STORE:
                    // For store, the value to be stored in main memory is
                    // in oper0, which was fetched in Decode.
                    memory[GlobalData.addrVal] = ins.getOper0().getValue();;
                    addStatusWord("Mem[" + GlobalData.addrVal + "]=" + ins.getOper0().getValueAsString());
                    return;
